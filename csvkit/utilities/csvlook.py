...
class CSVLook(CSVKitUtility):
    ...
    def main(self):
        table = agate.Table.from_csv(self.args.input_path, sniff_limit=self.args.sniff_limit, header=not self.args.no_header_row, column_types=self.get_column_types(), line_numbers=self.args.line_numbers, **self.reader_kwargs)
