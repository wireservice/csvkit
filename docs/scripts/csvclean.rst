...
-  If a CSV has columns that are empty in every data row, use :code:`--remove-empty-columns` to drop those columns from the output.
...
.. code-block:: none
   ...
   optional arguments:
     ...
     --remove-empty-columns
                           Remove columns that are empty in all data rows.
