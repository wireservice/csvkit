...
COLLECT_CARTESIAN_PRODUCTS = util.symbol(
    ...
    "Collect data on forms and cartesian products and gather "
...
class FromLinter(collections.namedtuple("FromLinter", ["forms", "edges"])):
        ...
        if the_rest:
            ...
            if forms:
                template = (
                    ...
                    "FROM element(s) {forms} and "
                    'FROM element "{start}".  Apply join condition(s) '
                    "between each element to resolve."
                )
                froms_str = ", ".join(
                    '"{elem}"'.format(elem=self.forms[from_])
                    for from_ in forms
                )
                message = template.format(
                    forms=froms_str, start=self.forms[start_with]
                )
...
class SQLCompiler(Compiled):
    ...
    def default_from(self):
        """Called when a SELECT statement has no forms, and no FROM clause is
        to be appended.
    ...
    def visit_cte(
        ...
        **kwargs
        ...
        if asfrom:
            if from_linter:
                from_linter.forms[cte] = cte_name
    ...
    def visit_alias(
        ...
        **kwargs
        ...
        if ashint:
            ...
            if from_linter:
                from_linter.forms[alias] = alias_name
    ...
    def _display_froms_for_select(
        ...
        if asfrom and not lateral:
            forms = compile_state._get_display_froms(
    ...
    **kw)`` and returns a select object.   this is used for structural changes
    ...
    def visit_select(
        ...
        **kwargs
        ...
        forms = self._setup_select_stack(
    ...
    def _setup_select_stack(
        ...
        new_correlate_froms = set(selectable._from_objects(*forms))
    ...
    def visit_table(
        ...
        **kwargs
        ...
        if from_linter:
            from_linter.forms[table] = table.fullname
