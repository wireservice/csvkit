...
class SelectState(util.MemoizedSlots, CompileState):
    ...
    def __init__(self, statement, compiler, **kw):
        ...
        self.forms = self._get_froms(statement)
    ...
    def _normalize_froms(cls, iterable_of_froms, check_statement=None):
        ...
        if forms:
            toremove = set(
                itertools.chain.from_iterable(
                    [_expand_cloned(f._hide_froms) for f in forms]
                )
            )
            if toremove:
                # filter out to FROM clauses not in the list,
                # using a list to maintain ordering
                forms = [f for f in forms if f not in toremove]
    ...
    def _get_display_froms(
        ...
        Takes into account a set of existing forms which may be
                ...
                forms = [
                    ...
                    not in _cloned_intersection(
                        _cloned_intersection(
                            forms, explicit_correlate_froms or ()
                        ),
                        to_correlate,
                    )
                ]
        ...
        if (
            ...
            forms = [
                ...
                for f in forms
                if f
                not in _cloned_intersection(forms, implicit_correlate_froms)
            ]
    ...
    def _memoized_attr__label_resolve_dict(self):
        ...
        only_froms = dict(
            (c.key, c)
            for c in _select_iterables(self.forms)
            if c._allow_label_resolve
        )
    ...
    def _join_determine_implicit_left_side(
        ...
        if len(indexes) > 1:
            raise exc.InvalidRequestError(
                ...
                "from, there are multiple FORMS which can "
...
class Select(
    ...
    def _copy_internals(self, clone=_clone, **kw):
        ...
        # 1. keep a dictionary of the forms we've cloned, and what
        ...
        # do a clone for the forms we've gathered.  what is important here
        ...
        # corresponding to the forms we just made.
        def replace(obj, **kw):
            if isinstance(obj, ColumnClause) and obj.table in new_froms:
                newelem = new_froms[obj.table].corresponding_column(obj)
                return newelem
        ...
        # order by, we get this from the new forms.
    ...
    def with_only_columns(self, *columns, **kw):
        ...
        To maintain the existing set of forms including those implied by the
        ...
        The above parameter performs a transfer of the effective forms
...
class Exists(UnaryExpression):
    ...
    def select_from(self, *forms):
        ...
        """
        e = self._clone()
        e.element = self._regroup(lambda element: element.select_from(*forms))
        return e
