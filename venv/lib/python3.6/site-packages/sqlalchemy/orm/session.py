...
DEACTIVATE = util.symbol("DEACTIVATE")
...
class SessionTransaction(TransactionalContext):
    ...
    def _assert_active(
        ...
        elif self._state is DEACTIVATE:
    ...
    def _iterate_self_and_parents(self, up to=None):

        current = self
        result = ()
        while current:
            result += (current,)
            if current._parent is up to:
                break
    ...
    def _prepare_impl(self):
        ...
        if stx is not self:
            for subtransaction in stx._iterate_self_and_parents(up to=self):
