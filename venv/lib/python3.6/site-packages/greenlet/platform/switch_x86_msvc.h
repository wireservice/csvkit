 ...
 * (SHE).
 *
 * SHE is implemented as a singly linked list of <function*, prev*> nodes. The
 ...
 * prev == 0xFFFFFFFF.
 *
 * SHE was designed for C, and traditionally uses the MSVC compiler
 * intrinsincs __try{}/__except{}. It is also utilized for C++ exceptions by
 * MSVC; there, every throw of a C++ exception raises a SHE error with the
 * ExceptionCode 0xE06D7363; the SHE handler list is then traversed to
 * deal with the exception.
 *
 * If the SHE list is corrupt, then when a C++ exception is thrown the program
 * will abruptly exit with exit code 1. This does not use std::terminate(), so
 * std::set_terminate() is useless to debug this.
 *
 * The SHE list is closely tied to the call stack; entering a function that
 ...
 * and restoring the head node of the SHE list (FS:[0]) per-greenlet is NOT
 * ENOUGH to make SHE or exceptions work.
 *
 * Stack switching breaks SHE because the call stack no longer necessarily
 * matches the SHE list. For example, given greenlet A that switches to
 * greenlet B, at the moment of entering greenlet B, we will have any SHE
 * handlers from greenlet A on the SHE list; greenlet B can then add its own
 * handlers to the SHE list. When greenlet B switches back to greenlet A,
 * greenlet B's handlers would still be on the SHE stack, but when switch()
 * returns control to greenlet A, we have replaced the contents of the stack
 * in memory, so all the address that greenlet B added to the SHE list are now
 * invalid: part of the call stack has been unwound, but the SHE list was out
 * of sync with the call stack. The net effect is that exception handling
 * stops working.
 *
 * Thus, when switching greenlets, we need to be sure that the SHE list
 ...
 *
 * The easiest way to do this is to capture the SHE list at the time the main
 * greenlet for a thread is created, and, when initially starting a greenlet,
 * start a new SHE list for it, which contains nothing but the handler
 * established for the new greenlet itself, with the tail being the handlers
 * for the main greenlet. If we then save and restore the SHE per-greenlet,
 * they won't interfere with each others SHE lists. (No greenlet can unwind
 * the call stack past the handlers established by the main greenlet).
 *
 * By observation, a new thread starts with three SHE handlers on the list. By
 ...
 * operates independently of the SHE handler list, and is called first.
 * Walking the SHE list at key points can also be helpful.
 *
 * References:
...
slp_set_exception_state(const void *const seh_state)
{
    ...
    // that one we want to keep, we need to relink the current SHE handler
    // frame to point to this one, cutting out the middle men, as it were.
    //
    // Entering a try block doesn't change the SHE frame, but entering a
...
x86_slp_get_third_oldest_handler()
{
    ...
    while (seh_state && seh_state != (GExceptionRegistration*)0xFFFFFFFF) {
        if ((void*)seh_state->prev < (void*)100) {
            fprintf(stderr, "\tERROR: Broken SHE chain.\n");
            return NULL;
        }
        a = b;
        b = c;
...
{
    // XXX: There appear to be three SHE handlers on the stack already at the
...
GreenletVectorHandler(PEXCEPTION_POINTERS ExceptionInfo)
{
    ...
    if (ExceptionRecord->NumberParameters == 3) {
        fprintf(stderr, "\tAbout to traverse SHE chain\n");
